...
for pom in [root_pom, *sorted(pathlib.Path(".").glob("*/pom.xml"))]:
    ...
    if root.findtext("m:artifactId", namespaces=ns) == "pipeline-blocks":
...
with tempfile.TemporaryDirectory(prefix="tpf-reactor-dependency-test-") as temporary:
    ...
    (root / "pom.xml").write_text("""<project><groupId>org.pipelineframework</groupId><artifactId>pipeline-blocks</artifactId><version>3.2.1-SNAPSHOT</version></project>""")
    (root / "consumer").mkdir()
    (root / "internal").mkdir()
    (root / "consumer/pom.xml").write_text("""<project><groupId>org.pipelineframework.blocks</groupId><artifactId>consumer</artifactId><parent><groupId>org.pipelineframework</groupId><artifactId>pipeline-blocks</artifactId><version>3.2.1-SNAPSHOT</version></parent><dependencies><dependency><groupId>org.pipelineframework.blocks</groupId><artifactId>internal</artifactId><version>3.2.1-SNAPSHOT</version></dependency><dependency><groupId>org.pipelineframework</groupId><artifactId>external</artifactId><version>3.2.1-SNAPSHOT</version></dependency></dependencies></project>""")
    (root / "internal/pom.xml").write_text("""<project><groupId>org.pipelineframework.blocks</groupId><artifactId>internal</artifactId><parent><groupId>org.pipelineframework</groupId><artifactId>pipeline-blocks</artifactId><version>3.2.1-SNAPSHOT</version></parent></project>""")
    ...
    assert dependencies[1].findtext("version") == "3.2.1-SNAPSHOT", ET.tostring(dependencies[1], encoding="unicode")
PY
    ;;
